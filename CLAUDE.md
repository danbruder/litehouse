...
- **Docker integration** - Container orchestration via Bollard (Docker API)
...
**Socket resolution:** Checks `DOCKER_HOST`, `DOCKER_SOCK` env vars, or defaults to `/var/run/docker.sock`.
...
- Socket path defaults to `/var/run/docker.sock` but can be overridden with `DOCKER_HOST` or `DOCKER_SOCK` environment variables
...
- `DOCKER_HOST`, `DOCKER_SOCK` - Override Docker socket path
