...
- Server-rendered admin UI (Askama + HTMX) served from the same binary, cookie-authenticated with the admin token. The dashboard at `/` is a React SPA (`frontend/`) instead — built once (never on the server) and its output committed into `src/ui/spa/`, embedded with `include_str!` like `htmx.min.js`/`styles.css`. Everything else (`/apps/:name`, `/backups`, deploy detail) is still Askama+HTMX. See `frontend/README.md` before touching either the dashboard or the two new SPA-only API endpoints under `/api/apps/summary`, `/api/apps/:name/restart`, `/api/metrics/server`.
